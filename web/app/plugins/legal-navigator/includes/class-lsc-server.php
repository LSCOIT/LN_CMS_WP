...
class LSC_Server {
	public function api_request( $scope_id, $path, $data = [] ) {
	...
	private function get_server_by_id( $scope_id ) {
		...
		$key    = array_search( $scope_id, array_column( $servers, 'connection_scope_id' ), true );
		$server = $servers[ $key ];
	...
	private function get_auth_token( $server ) {
		...
		if ( wp_remote_retrieve_response_code( $auth_request ) === 200 ) {
	...
	private function update_upload_date(): void {
		$servers = get_field( 'connections', 'option' );
		$organizational_units = get_organizational_unit();
		$default_params = [ 'sslverify' => true, 'timeout' => 60 ];
		...
		foreach ($servers as $server) {
			foreach ( array_keys( $organizational_units ) as $organizational_unit ) {
				...
				$topics_request_url = add_query_arg( $query_params, $server['connection_url'] . 'api/topics-resources/topics' );
				$response           = wp_remote_get( $topics_request_url, $default_params );
				...
				if ( wp_remote_retrieve_response_code( $response ) === 200 ) {
					...
					if ( $topics ) {
						foreach ( $topics as $topic ) {
							...
							if ( ! empty( $topic ) ) {
								$time  = date( 'Y-m-d H:i:s', date( 'U', strtotime( $topic['modifiedTimeStamp'] ) ) );
								$term_id = get_topic_id_by_uid($topic['id']);
								if ($term_id) {
									update_term_meta($term_id, '_scope_' . $server['connection_scope_id'], $time);
								}
								...
								$resources_request_url = add_query_arg( $query_params, $server['connection_url'] . 'api/topics-resources/resources' );
								$response              = wp_remote_get( $resources_request_url, $default_params );
								...
								if ( wp_remote_retrieve_response_code( $response ) === 200 ) {
									...
									if ( ! empty( $resources ) ) {
										foreach ( $resources as $resource ) {
											if ( ! empty( $resource ) ) {
												$time  = date( 'Y-m-d H:i:s', date( 'U', strtotime( $resource['modifiedTimeStamp'] ) ) );
												$post_id = get_post_id_by_uid($resource['id']);
												if ($post_id) {
													update_post_meta($post_id, '_scope_' . $server['connection_scope_id'], $time);
												}
											}
										}
									}
	...
	public function save_topic( $topic ): void {
		...
		if ( ! empty( $topic['parentTopicId'] ) ) {
			...
			if ( ! $parent_id && $saved_id = $this->save_topic_by_uid( $topic_uid ) ) {
				$parent_id = $saved_id;
			}
		...
		if ( $topic_id ) {
			...
			$created_by = $user->ID ?? get_current_user_id();
			...
			$modified_by = $user->ID ?? get_current_user_id();
	...
	private function save_topic_by_uid( $topic_uid ) {
	...
	private function transform_resource_locations( $resource_locations, $type = 'topic' ): array {
		$locations = [];
	...
	private function save_resource( $resource ): void {
		...
		remove_filter( 'wp_insert_post_data', 'lsc_alter_post_modification_time', 99 );
	...
	private function save_page( $page ): void {
		$page_data = array(
	...
	public function get_curated_experiences( $scope_id, $unit = null ) {
		$server = $this->get_server_by_id( $scope_id );
	...
	public function delete_curated_experience( $scope_id, $ce ): ?bool {
		$server = $this->get_server_by_id( $scope_id );
	...
	public function upload_curated_experience( $scope_id, $data ) {
		$server = $this->get_server_by_id( $scope_id );
